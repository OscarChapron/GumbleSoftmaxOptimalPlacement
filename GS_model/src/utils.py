...
def get_constant_time_wei(patch_dims, offset=0, **crop_kw):
    # Get constant crop weight based on patch dimensions and additional crop keywords
    pw = get_constant_crop(patch_dims, **crop_kw)
    
    # Generate a constant weight array across the time window
    return np.ones(tuple(patch_dims.values())) * pw
