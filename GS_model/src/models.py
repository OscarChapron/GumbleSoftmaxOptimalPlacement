...
class GradSolver(nn.Module):
    ...
    def solver_step(self, state, batch, step):
        ...
        return state - state_update, grad
    
    #def forward(self, batch):
    #    with torch.set_grad_enabled(True):  # Ensure gradients are always enabled
    #        state = self.init_state(batch)
    #        self.grad_mod.reset_state(batch.input)
#
    #        for step in range(self.n_step):
    #            state = self.solver_step(state, batch, step=step)
    #            # Removed the conditional detachment and re-enabling of gradients
#
    #        # The condition for applying prior_cost in eval mode is removed
    #        # to ensure consistent behavior in terms of gradient computation
    ...
    def forward(self, batch):
        with torch.set_grad_enabled(True):
            ...
            for step in range(self.n_step):
                state, grad = self.solver_step(state, batch, step=step)
