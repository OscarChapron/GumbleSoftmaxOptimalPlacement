...
class BaseDataModule_comfra(pl.LightningDataModule):
    ...
    #def norm_stats(self):
    #    if self._norm_stats is None:
    #        seasons = self.input_da.sel(self.xrds_kw.get('domain_limits', {})).sel(self.domains['train']).groupby('time.season')
    #        norm_stats = {}
    #        for season, group in seasons:
    #            if self.norm_type == 'z_score':
    #                mean = group.mean().values.item()
    #                std = group.std().values.item()
    #                norm_stats[season] = (mean, std)
    #            elif self.norm_type == 'min_max':
    #                min_val = group.min().values.item()
    #                max_val = group.max().values.item()
    #                norm_stats[season] = (min_val, max_val)
    #        self._norm_stats = norm_stats
    #        print("Norm stats", self._norm_stats)
    #    return self._norm_stats
